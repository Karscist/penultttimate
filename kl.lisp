...
(defpackage "KL"
  ...
  (:export
   ...
   :geta :seta
   :make-simple-object :make-simple-object-data
   :*stack* :s :spop :with-stack
   :ref :deref :setref
   :read-input
   :get-var :get-var-optional :get-var! :get-var-optional!
   :point
   :aset :vset
   :make-vector :vector-map :vector-foldr
   :array-foldr :array-map-2d :array-2d>list
   :is-object
   :deque-element :deque))
(in-package "KL")
       ...
       (push λs.λpx(progn
                     (cond
                       ...
                       ((> p intpos)
                        ...
                        #*(s 'push p x)))
                     s))
       (popl λ_.λ(when (> length 0)
                   (let-n (out nh) (#*(head 'get) #*(head 'next))
                     (if nh
                         (progn
                           #*(head 'splitr nh)
                           (setq head nh)
